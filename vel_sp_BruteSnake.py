...
class OffboardControlNode(Node):
    ...
    def velocity_update_callback(self):
        ...
        if current_mode == 'OFFBOARD':
            if time_since_last_req > Duration(seconds=5) and not hover_sent:
                with self.lock:
                    self.HoverSent = True
                    self.vel_cmd.linear.z = 0.0
                    self.vel_cmd.linear.y = 0.0
                    self.vel_cmd.linear.x = 0.0
            if time_since_last_req > Duration(seconds=15):
                with self.lock:
                    self.vel_cmd.linear.x = 1.0
                    self.vel_cmd.linear.z = 0.0
                    self.vel_cmd.linear.y = 0.0
            if time_since_last_req > Duration(seconds=25):
                with self.lock:
                    self.vel_cmd.linear.x = 0.0
                    self.vel_cmd.linear.z = 0.0
                    self.vel_cmd.linear.y = 1.0
            if time_since_last_req > Duration(seconds=35):
                with self.lock:
                    self.vel_cmd.linear.x = 1.0
                    self.vel_cmd.linear.z = 0.0
                    self.vel_cmd.linear.y = 0.0
            if time_since_last_req > Duration(seconds=45):
                with self.lock:
                    self.vel_cmd.linear.x = 0.0
                    self.vel_cmd.linear.z = 0.0
                    self.vel_cmd.linear.y = -2.0
            if time_since_last_req > Duration(seconds=55):
                with self.lock:
                    self.vel_cmd.linear.x = 1.0
                    self.vel_cmd.linear.z = 0.0
                    self.vel_cmd.linear.y = 0.0
            if time_since_last_req > Duration(seconds=65):
                with self.lock:
                    self.vel_cmd.linear.x = 0.0
                    self.vel_cmd.linear.z = 0.0
                    self.vel_cmd.linear.y = 2.0
            if time_since_last_req > Duration(seconds=75):
                with self.lock:
                    self.vel_cmd.linear.x = 1.0
                    self.vel_cmd.linear.z = 0.0
                    self.vel_cmd.linear.y = 0.0
            if time_since_last_req > Duration(seconds=85):
                with self.lock:
                    self.vel_cmd.linear.x = 0.0
                    self.vel_cmd.linear.z = 0.0
                    self.vel_cmd.linear.y = -1.0
    ...
    def landing_check_callback(self):
        ...
        if current_mode == 'OFFBOARD' and time_since_last_req > Duration(seconds=95):
