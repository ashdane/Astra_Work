...
class OffboardControlNode(Node):
    def __init__(self):
        ...
        # Separate timers for different tasks
        self.create_timer(1.0, self.state_transition_callback)  # State management
        self.create_timer(0.05, self.velocity_update_callback)  # Velocity updates
        self.create_timer(1.0, self.landing_check_callback)     # Landing checks
    ...
    def velocity_update_callback(self):
        ...
        if current_mode == 'OFFBOARD':
            if time_since_last_req > Duration(seconds=10) and not hover_sent:
                with self.lock:
                    self.HoverSent = True
                    self.vel_cmd.linear.z = 0.0
                    self.vel_cmd.linear.x = 0.0
                    
            if time_since_last_req > Duration(seconds=20):
                with self.lock:
    ...
    def landing_check_callback(self):
        ...
        if current_mode == 'OFFBOARD' and time_since_last_req > Duration(seconds=50):
